...
Crash_Ped %>%
  select(geometry) %>%
  st_transform('ESRI:102271')%>%
  mutate(CrashCount1 = 1) 

Crash_geoid <-
  st_join(Crash_geo, Agg_above_60,join=st_within)%>%
  select(-Age_Above60)%>%
  st_drop_geometry()%>%
  group_by(GEOID)%>%
  summarize(CrashCount = sum(CrashCount1))%>%
  left_join(.,block_geo) %>%
  st_as_sf()%>%
...
ggplot()+
  ...
  geom_sf(data = Crash_geoid, aes(fill = CrashCount))+
...
Tree_geoid <-
  st_join(Tree_geo, Agg_above_60_4326,join=st_within)%>%
  select(-Age_Above60)%>%
  st_drop_geometry()%>%
  group_by(GEOID)%>%
  summarize(TreeCount = sum(TreeCount1))%>%
  left_join(.,block_geo) %>%
  st_as_sf()%>%
  st_transform(st_crs = fishnet)
...
tree_no_geo <- 
  st_drop_geometry(Tree_geoid) 

white_no_geo <-
  st_drop_geometry(Philly_White)

black_no_geo <-
  st_drop_geometry(Philly_Black)

asian_no_geo <-
  st_drop_geometry(Philly_Asian)

no_car_no_geo <-
  st_drop_geometry(No_car_commute)

total_no_geo <-
  st_drop_geometry(Philly_Total)
...
vars_net <- 
  rbind(Agg_above_60,Agg_age_below_10,Crash_geoid2,No_car_commute, Tree_geoid2,Philly_White,Philly_Asian,Philly_Black,Philly_Total)%>%
  left_join(.,block_geo)%>%
...
Crime <-
  rbind(Crime_2020,Crime_2019,Crime_2018, Crime_2017, Crime_2016) %>%na.omit()%>%
   st_as_sf(coords = c("point_x","point_y"),crs = "ESRI:102271")

Crime_Veh <-
  Crime %>%
  filter (text_general_code == "Theft from Vehicle") %>%
  select(geometry)%>%
  mutate(CrimeCount = 1)

Crime_Veh2 <-
  st_join(Crime_Veh, block_geo, st_join = within)

s
%>%
    st_drop_geometry()%>%
  group_by(GEOID)%>%
  summarize(CrimeCount = sum(CrimeCount1))%>%
  left_join(.,block_geo) %>%
  st_as_sf()
